...
<div class="btn btn-control bg-primary more"> <svg class="olymp-settings-icon">
  ...
  <ul id="cover_photo_option" class="more-dropdown more-with-triangle triangle-bottom-right">
    ...
    <?php if($member['cover_img']!='') { ?>
	<li><a href="#" id="remove_header_photo">Remove Cover Photo</a></li>
    <?php }?>
    
   </ul>
   
  <form id="cover_image_frm" action="<?=file_path('upload_img/upload_cover_image')?>" method="post" enctype="multipart/form-data">
    <input type="hidden" name="id" value="1">
    <input type="hidden" name="imgPre" value="member_cover">
    <input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash();?>">
    <input type="file" value="upload" id="cover_image" name="cover_image" style="display:none;">
  </form>
  <form id="profile_image_frm" action="<?=file_path('upload_img/upload_profile_image')?>" method="post" enctype="multipart/form-data">
    <input type="hidden" name="id" value="1">
    <input type="hidden" name="imgPre" value="member_profile">
    <input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash();?>">
...
<script nonce=<?=SC_NONCE?>>
	...
	$(document).on('click','#header_photo',function(e){
        ...
        $(document).on('click', '#remove_header_photo', function (e) {
			...
			var action_url = '<?=file_path('profile/remove_cover_photo')?>';
        ...
        $(document).on('click', '#remove_profile_photo', function (e) {
			...
			var action_url = '<?=file_path('profile/remove_profile_photo')?>';
