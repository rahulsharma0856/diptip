   ...
   <!----Top Detail----> 
    ...
    <div class="post__author author vcard inline-items"> 
                ...
                <?php if($result['post_category']=='group') { ?>
                    ...
                    <a class="author_nm h6 post__author-name fn 2" href="<?=$result['detail']['r_post_by_url']?>"><?=$result['detail']['r_post_by']?></a>  <?php if($section=='group'){ ?><?=$tagHtml?><?php } ?>
                    
                     <?php if($section!='group'){ ?>
                    
                            &nbsp;&nbsp; <i class="fa fa-chevron-right"></i>&nbsp;&nbsp;  
                            
                            <a class="author_nm h6 post__author-name fn 1" href="<?=file_path('group/view/'.$result['group_page_id'])?>"><?=$result['gp_name']?></a> <?=$tagHtml?>
                ...
                <?php } elseif($result['post_category']=='tag') { ?>
                	...
                	<a class="author_nm h6 post__author-name fn 3" href="<?=$result['detail']['r_post_by_url']?>"><?=$result['detail']['r_post_by']?></a> 
                    
                    
                      &nbsp;&nbsp; <i class="fa fa-chevron-right"></i>&nbsp;&nbsp;  
                    
                    
                    <a class="author_nm h6 post__author-name fn 3" href="<?=$result['detail']['tag_url']?>"><?=$result['detail']['tag_to']?></a>   
                    
                
                <?php } else{?>
                
                		<a class="author_nm h6 post__author-name fn 3" href="<?=$result['detail']['r_post_by_url']?>"><?=$result['detail']['r_post_by']?></a> <?=$tagHtml?>
      ...
      <?php } ?>
		...
		<?php
		if(preg_match('/iframe/',$result['post_text'])){
				
			$input_text = '<pre style="white-space: pre-wrap;font-size: 14px;color: #000;">'.htmlspecialchars($result['post_text']).'</pre>';
			
		}
		else
		{
			 $input_text = nl2br($result['post_text']);
		}
		...
		if($result['share_url']!=""){ ?>
		    ...
		    <?php 
				...
				//$share_url_info = stripslashes(html_entity_decode($result['share_url_info']));
			
				//$share_url_info = json_decode(trim($share_url_info),true); 
				
				$share_url_info = json_decode(trim($result['share_url_info']),true); 
        ...
        <div class="post-additional-info inline-items"> 
           	...
           	<?php if((int)$summary['is_like'] > 0) {?>
                ...
                <span class="post_do_like" id="sp_po_like<?=$result['post_id']?>">
                	<span>
                        <a href="#" class="post-add-icon inline-items" id="do_unlike_post" value="<?=$result['post_id']?>"> 
      ...
      <form class="comment-form inline-items" method="post" id="post_comment_frm" style="border-top:none;border-top:none;padding: 10px 25px 10px 25px;">
        ...
        <div class="post__author author vcard inline-items">
        	...
        	<img src="<?=thumb(ProfileImg(user_session('profile_pic')),150,150)?>" alt="author"> 
        	<?php /*?><img src="<?=thumb(ProfileImg($result['detail']['r_profile_img']),100,100)?>" alt="author 2"><?php */?>
        </div>
        
        <div class="form-group with-icon-right is-empty">
        
        	<textarea placeholder="Write a comment..." style="height: 35px;min-height: 35px;overflow: hidden;padding: 5px;" class="form-control post_comment_box" id="pcomment" name="pcomment" onKeyUp="countChar(this)"></textarea>
