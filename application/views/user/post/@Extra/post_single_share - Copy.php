  ...
  <article class="hentry post has-post-thumbnail"> 
    ...
    <div class="post__author author vcard inline-items">
      ...
      <div class="author-date">
        ...
        <a class="author_nm h6 post__author-name fn 2" href="<?=file_path('profile/view/'.$result['group_member_username'])?>">
        ...
        <?php } ?>
        <?php }  else { ?>
        <a class="author_nm h6 post__author-name fn 2" href="<?=$result['detail']['r_post_by_url']?>">
        <?=$result['detail']['r_post_by']?>
        </a>
        <?php } ?>
        <span style="color:#6c6c6d"> shared a </span><a style="color:#47a247" href="#">Post</a>
    ...
    <?php
	if(preg_match('/iframe/',$result['share_txt'])){
			
		$input_text = '<pre style="white-space: pre-wrap;font-size: 14px;color: #000;">'.htmlspecialchars($result['share_txt']).'</pre>';
		
	}
	else
	{
		 $input_text = nl2br($result['share_txt']);
	}
    ...
    <p style="color:#000;word-break:break-word;">
      <?= getUrls($input_text)?>
        ...
        <?php $share_url_info = json_decode($result['share_url_info'],true); ?>
   ...
   ?>
    <ul class="children single-children">
        ...
        <?php if($result['is_ads']=='1'){ 
        
        		$ads = $this -> comman_fun -> get_table_data('social_ads_create',array('id'=>$result['ads_code']));
       ...
       <?php   /*?> <img src="<?=thumb($shared['detail']['r_profile_img'],100,100)?>" alt="author"><?php */?>
        ...
        <img src="<?=thumb(ProfileImg($shared['detail']['r_profile_img']),100,100)?>" alt="author1">
        
          <div class="author-date">
          
            <?php if($shared['post_category']=='group'){ ?>
            
            	<a class="h6 post__author-name fn" href="<?=file_path('profile/view/'.$shared['group_member_username'])?>">
            
            		<?=$shared['detail']['r_post_by']?>
            
            	</a> &nbsp;&nbsp;<i class="fa fa-chevron-right"></i>&nbsp;&nbsp; <a class="h6 post__author-name fn" href="<?=file_path('group/view/'.$shared['group_page_id'])?>">
            	<?=$shared['gp_title']?>
            </a>
            <?=$tagHtml?>
            ...
            <a class="h6 post__author-name fn" href="<?=$shared['detail']['r_post_by_url']?>">
            <?=$shared['detail']['r_post_by']?>
            </a> &nbsp;&nbsp; <i class="fa fa-chevron-right"></i>&nbsp;&nbsp; <a class="author_nm h6 post__author-name fn 3" href="<?=$shared['detail']['tag_url']?>">
    ...
    <div class="post-additional-info inline-items">
      ...
      <span id="sp_po_like<?=$result['post_id']?>"> <span> <a href="#" class="post-add-icon inline-items" id="do_unlike_post" value="<?=$result['post_id']?>"> <i class="fa fa-heart"></i></a> </span> <span> <a href="<?=file_path('dashboard/getWhoPostLikesMember/'.$result['post_id'])?>" class="who-likes-popover post-add-icon like-post-<?=$result['post_id']?>" id="who-likes-popover">
      <?=$summary['total_likes']?>
      Like </a> </span> </span>
      <?php } else { ?>
      <span id="sp_po_like<?=$result['post_id']?>"> <span> <a href="#" class="post-add-icon inline-items" id="do_like_post" value="<?=$result['post_id']?>"> <i class="fa fa-heart-o fa fa-heart"></i> </a> </span> <span> <a href="<?=file_path('dashboard/getWhoPostLikesMember/'.$result['post_id'])?>" class="who-likes-popover post-add-icon like-post-<?=$result['post_id']?>" id="who-likes-popover">
      <?=$summary['total_likes']?>
      Like </a> </span> </span>
      <?php } ?>
      <div class="comments-shared"> <span><a id="btn_post_comment" value="<?=$result['post_id']?>" href="#" class="post-add-icon inline-items"> <svg class="olymp-speech-balloon-icon">
        <use xlink:href="<?=asset_sm()?>icons/icons.svg#olymp-speech-balloon-icon"></use>
        </svg> <span><span class="total_comments">
        <?=$summary['total_comments']?>
        </span> Comment</span> </a> </span> <span class="more"> <span class="post-add-icon inline-items"> <svg class="olymp-share-icon">
        ...
        <ul class="more-dropdown">
          ...
          <li><a class="share_post_link" href="<?=file_path('post/share_popup/'.$result['post_id'].'/page')?>" value="2">Share on Page</a> </li>
          <li><a class="share_post_link" href="<?=file_path('post/share_popup/'.$result['post_id'].'/group')?>" value="3">Share on Group</a> </li>
        </ul>
        </span> </span>
        <?php /*?> <span>
