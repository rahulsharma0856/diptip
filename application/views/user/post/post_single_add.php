...
<div class="ui-block post-message" id="post<?=$result['post_id']?>" data-id="<?=$result['post_id']?>" style="border: 1px solid #e1e1e3!important;border-radius: 2px;">
...
<?php } ?>
  <article class="hentry post has-post-thumbnail">
   ...
   <?php
   	...
   	$CountPostTaggedMember = $this->Post_model->getCountPostTaggedMember($result['post_id']);
	
	if($CountPostTaggedMember > 0){
		
		$PostTaggedMember = $this->Post_model->getPostTaggedMember($result['post_id'],1);
		...
		if($CountPostTaggedMember == 1){
			...
			$tagHtml = 'is with <a href="#" class="theme-txt1">'.$PostTaggedMember[0]['name'].'</a> and <a class="notifiction-popover theme-txt1" data-placement="bottom" href="'.file_path('tag/taggedmembertist/'.$result['post_id']).'">'.$otherMemberCount.' other </a>';
    ...
    <div class="post__author author vcard inline-items"> 
               ...
               <img src="<?=thumb(ProfileImg($result['detail']['r_profile_img']),100,100)?>" alt="author" title="Post Author:  <?=$result['detail']['r_post_by']?>">
                ...
                <?php if($result['post_category']=='group') { ?>
                    ...
                    <a class="author_nm h6 post__author-name fn 2" href="<?=$result['detail']['r_user_url']?>" title="Post Author:  <?=$result['detail']['r_post_by']?>"><?=$result['detail']['r_post_by']?></a>  <?php if($section=='group'){ ?><?=$tagHtml?><?php } ?>
                    
                     <?php if($section!='group'){ ?>
                    
                            &nbsp;&nbsp; <i class="fa fa-chevron-right"></i>&nbsp;&nbsp;  
                            
                            <a class="author_nm h6 post__author-name fn 1" href="<?=file_path('group/view/'.$result['group_page_id'])?>" title="Post Author:  <?=$result['gp_name']?>"><?=$result['gp_name']?></a> <?=$tagHtml?>
                    
                    <?php } ?>
                    
                  
                
                <?php } elseif($result['post_category']=='tag') { ?>
                
                
                	<a class="author_nm h6 post__author-name fn 3" href="<?=$result['detail']['r_post_by_url']?>" title="Post Author:  <?=$result['detail']['r_post_by']?>"><?=$result['detail']['r_post_by']?></a> 
                    
                    
                      &nbsp;&nbsp; <i class="fa fa-chevron-right"></i>&nbsp;&nbsp;  
                    
                    
                    <a class="author_nm h6 post__author-name fn 3" href="<?=$result['detail']['tag_url']?>" title="Post Author:  <?=$result['detail']['tag_to']?>"><?=$result['detail']['tag_to']?></a>   
                    
                
                <?php } else{?>
                
                		<a class="author_nm h6 post__author-name fn 3" href="<?=$result['detail']['r_post_by_url']?>" title="Post Author:  <?=$result['detail']['r_post_by']?>"><?=$result['detail']['r_post_by']?></a> <?=$tagHtml?>
                ...
                <time class="published" datetime="2004-07-24T18:18"> 
                	<a class="post_time" href="<?=file_path('dashboard/post/'.$result['post_id'])?>">
	  ...
	  <svg class="olymp-three-dots-icon"><use xlink:href="<?=asset_sm()?>icons/icons.svg#olymp-three-dots-icon"></use></svg>
        ...
        <ul class="more-dropdown">
          ...
          <?php
			...
			{
				$public_clr = "style='color: #00547b!important;'";
				
				$privacy_sel_public = "class='privacy_sel'";
			}
			else if($result['privacy']=='Private')
			{
				$private_clr = "style='color: #00547b!important;'";
				
				$privacy_sel_private = "class='privacy_sel'";
			}
			else if($result['privacy']=='Friends')
			{
				$friends_clr = "style='color: #00547b!important;'";
				
				$privacy_sel_friends = "class='privacy_sel'";
			}
			
		  ?>
          <?php if($result['post_category']=='member') {?>
		 
          <li> <a id="change_privacy" <?=$privacy_sel_public?> <?=$public_clr?> href="<?=file_path('post/change_privacy/Public/'.$result['post_id'])?>">Public</a> </li>
          <li> <a id="change_privacy" <?=$privacy_sel_private?> <?=$private_clr?> href="<?=file_path('post/change_privacy/Private/'.$result['post_id'])?>">Private</a> </li>
          <li> <a id="change_privacy" <?=$privacy_sel_friends?> <?=$friends_clr?> href="<?=file_path('post/change_privacy/Friends/'.$result['post_id'])?>">Friends</a> </li>
      ...
      <?php } ?>
        ...
        <?php if($result['share_url']!=""){ ?>
            ...
            <p style="margin: 0px;font-weight: bold;color:#888da8;word-break: break-word;"><?=strtoupper($share_url_info['host'])?><br> <?=$share_url_info['title']?></p>
        ...
        <div class="post-additional-info inline-items"> 
           	...
           	<?php if((int)$summary['is_like'] > 0) {?>
                ...
                <span class="post_do_like" id="sp_po_like<?=$result['post_id']?>">
                	<span>
                        <a href="#" class="post-add-icon inline-items" id="do_unlike_post" value="<?=$result['post_id']?>" title="Unlike Post"> 
                            <i class="fa fa-heart"></i> 
                        </a>
                    </span>
                    
                    <span>
						<a href="<?=file_path('dashboard/getWhoPostLikesMember/'.$result['post_id'])?>" class="who-likes-popover post-add-icon like-post-<?=$result['post_id']?>" id="who-likes-popover" title="<?=$summary['total_likes']?> Likes"> 
            ...
            <?php } else { ?>
                ...
                <span class="post_do_like" id="sp_po_like<?=$result['post_id']?>" title="Like Post">
                	<span>
                    	<a href="#" class="post-add-icon inline-items" id="do_like_post" value="<?=$result['post_id']?>"><i class="fa fa-heart-o fa fa-heart"></i></a> 
                    </span>
					<span>
                        <a href="<?=file_path('dashboard/getWhoPostLikesMember/'.$result['post_id'])?>" class="who-likes-popover post-add-icon like-post-<?=$result['post_id']?>" id="who-likes-popover" title="<?=$summary['total_likes']?> Likes"> 
							<?php if ($summary['total_likes'] == 1) {?>
            ...
            <div class="comments-shared" style="margin-top: 0px;"> 
                ...
                <span class="total_comments" id="total_post_total_comments_<?=$result['post_id']?>"><?=$summary['total_comments']?> <?php if($summary['total_comments'] != 1) {?>Comments<?php } else {?>Comment<?php }?></span> </a> </span>
                
                 <span class="more">
                
                    <span class="post-add-icon inline-items">
                    
                    <svg class="olymp-share-icon"> <use xlink:href="<?=asset_sm()?>icons/icons.svg#olymp-share-icon"></use></svg>  <?=$summary['total_share']?> <?php if($summary['total_share'] != 1) {?>Shares<?php } else {?>Share<?php }?>
