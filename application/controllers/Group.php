...
class Group extends App
{
    ...
    public function add()
    {
        ...
        $data["category"] = $this->comman_fun->get_table_data(
            "sm_page_category",
            ["status" => "Active"]
        );
    ...
    protected function _insert()
    {
        ...
        if ($_POST["mode"] == "edit") {
            $this->comman_fun->update($data, "social_page_group", [
                "id" => $_POST["eid"],
            ]);
    ...
    public function view($id = null)
    {
        ...
        if (isset($data["result"][0])) {
            ...
            $data["JoinedGroups"]  = $this->Group_model->getMemberJoinedGroups(
    ...
    public function isValidView($data)
    {
        if ($data["result"][0]["group_privacy"] == "Public") {
            return true;
        } else {
            if (
                $data["isPageAdmin"] == true ||
                $data["isGroupJoined"] == true
            ) {
                return true;
            }
        }
    ...
    public function requests($id = null)
    {
        ...
        $data["GroupJoinRequest"] = $this->Group_model->getGroupJoinRequest(
            $id
        );
    ...
    public function join_request_delete($id)
    {
        $request = $this->Group_model->getJoinRequestById($id);

        if (isset($request[0])) {
            if ($this->Group_model->isAdmin($request[0]["pg_code"])) {
                $this->comman_fun->delete("social_page_group_member", [
                    "id" => $request[0]["id"],
                ]);

                $this->comman_fun->delete("social_notification", [
                    "usercode2" => $request[0]["usercode"],
    ...
    public function join_request_accept($id)
    {
        $request = $this->Group_model->getJoinRequestById($id);
    ...
    public function members($id = null)
    {
        ...
        $data["JoinedMembers"] = $this->Group_model->getGroupJoinedMemberList(
            $id
        );
    ...
    public function ajax_timeline_post()
    {
        ...
        $result = $this->Post_model->getGroupTimelinePost(
            $_GET["u"],
            $start_from
        );
        ...
        for ($i = 0; $i < count($result); $i++) {
            $html .= $this->load->view(
                "user/post/post_single",
                ["result" => $result[$i], "section" => "group"],
                true
            );
        }
    ...
    public function checkNewMessages()
    {
        ...
        if (isset($_GET["uid"]) && isset($_GET["last"])) {
            ...
            if (
                $this->Post_model->checkGrouptimelineNewMessages(
                    $_GET["uid"],
                    $last
                )
            ) {
                $result = $this->Post_model->getGrouptimelineNewMessages(
                    $_GET["uid"],
                    $last
                );

                for ($i = 0; $i < count($result); $i++) {
                    $data["post" . $result[$i]["post_id"]] = $this->load->view(
                        "user/post/post_single",
                        ["result" => $result[$i], "section" => "group"],
                        true
                    );
    ...
    public function send_join_request($id, $view = "view1")
    {
    ...
    public function ajaxJosnGroupHtml2($id)
    {
        ...
        $data["html"] = $this->load->view(
            "user/search/group_html",
            ["result" => $result[0], "only_inner" => true],
            true
        );
    ...
    public function delete_request($id, $view = "view1")
    {
    ...
    public function member_group_status($id)
    {
        if ($this->Group_model->isGroupJoined($id)) {
            $html =
                '<div class="btn btn-control bg-green more reqicon" id="delete_request" value="' .
                $id .
                '" title="Joined Group"><a href="#"><i class="fa fa-user"></i></a></div>';
        } elseif ($this->Group_model->isRequestPending($id)) {
            $html =
                '<div class="btn btn-control bg-purple more reqicon" id="delete_request" value="' .
                $id .
                '" title="Request Pending"><a href="#"><i class="fa fa-user-times"></i></a></div>';
        } elseif ($this->Group_model->isGroupInvitation($id)) {
            $html =
                '<div style="font-size: 12px;" class="btn btn-green btn-sm" id="accept_group_request" value="' .
                $id .
                '" title="Accept Join Invitation"><a style="color:#fff;" href="#"><i style="margin-right: 10px;" class="fa fa-user-plus"></i>Accept Join Invitation</a></div>';
        } else {
            $html =
                '<div class="btn btn-control bg-primary more reqicon" id="request_send" value="' .
                $id .
                '" title="Send Join Group Request"><a href="#"><i class="fa fa-user-plus"></i></a></div>';
        }
    ...
    public function remove_cover_photo($group_id)
    {
        $record = $this->comman_fun->get_table_data("social_page_group", [
            "id"         => $group_id,
            "type"       => "group",
            "created_by" => user_session("usercode"),
        ]);
    ...
    public function remove_profile_photo($group_id)
    {
        $record = $this->comman_fun->get_table_data("social_page_group", [
            "id"         => $group_id,
            "type"       => "group",
    ...
    public function delete_my_group($id)
    {
        if ($this->Group_model->isAdmin($id)) {
            ...
            if (isset($g_member_list[0])) {
                for ($i = 0; $i < count($g_member_list); $i++) {
                    $this->comman_fun->delete("social_page_group_member", [
                        "pg_code"  => $id,
                        "usercode" => $g_member_list[$i]["usercode"],
                        "type"     => "group",
                    ]);
                }
            }
            ...
            $record = $this->comman_fun->get_table_data("social_post_master", [
                "group_page_id" => $id,
                "post_category" => "group",
            ]);
            ...
            if (isset($record[0])) {
                for ($i = 0; $i < count($record); $i++) {
                    $this->comman_fun->delete("social_posts", [
                        "id" => $record[$i]["post_code"],
                    ]);
                }

                $this->comman_fun->delete("social_post_master", [
                    "group_page_id" => $id,
                    "post_category" => "group",
                ]);
            }

            $isDelete = $this->comman_fun->get_table_data("social_page_group", [
                "id"     => $id,
                "status" => "Delete",
            ]);
    ...
    public function accept_group_join_request($id)
    {
        if ( ! $this->Group_model->isGroupJoined($id)) {
            $data = [
                "type"     => "group",
            ...
            $insert_id = $this->comman_fun->addItem(
                $data,
                "social_page_group_member"
            );

            if (isset($insert_id)) {
                $this->comman_fun->delete("social_invite", [
                    "pg_id"    => $id,
                    "usercode" => user_session("usercode"),
                ]);
