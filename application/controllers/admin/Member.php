...
class Member extends CI_Controller
{
    ...
    public function listing()
    {
        ...
        $output = ["sEcho" => intval($_GET['sEcho']), "iTotalRecords" => $count[0]['tot'], "iTotalDisplayRecords" => '' . $count[0]['tot'] . '', "aaData" => []];
        for ($i = 0; $i < count($result); $i++) {
            $btn                = '<a  href="' . file_path('admin/member/view') . $result[$i]['usercode'] . '"><i class="fa fa-eye"></i></a>';
            $row                = [$result[$i]['usercode'], $result[$i]['fname'] . ' ' . $result[$i]['lname'], $result[$i]['username'], $result[$i]['emailid'], $result[$i]['create_date'], $btn];
            $output['aaData'][] = $row;
        }
