...
class Dashboard extends CI_Controller
{
    ...
    public function profile_edit_insert()
    {
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if ($this->form_validation->run() === false) {
                ...
                $this->session->set_flashdata('show_msg', ['class' => 'true', 'msg' => 'Profile Update Successfully.....']);
    ...
    protected function _profile_edit_insert()
    {
        ...
        $this->comman_fun->update($data, 'membermaster', ['usercode' => $_POST['usercode']]);
        $data = false;
    }
    public function check_emailid()
    {
        if ($this->comman_fun->check_record('membermaster', ['usercode !=' => $_POST['usercode'], 'emailid' => $_POST['emailid']])) {
    ...
    {
        if ($this->comman_fun->check_record('membermaster', ['usercode !=' => $_POST['usercode'], 'username' => $_POST['username']])) {
    ...
    public function disable_2fa($member_id)
    {
        ...
        $this->session->set_flashdata('show_msg', ['class' => 'true', 'msg' => 'Disable Two Factor Authentication Successfully']);
        header('Location: ' . file_path('admin') . '' . $this->uri->rsegment(1) . '/member/' . $member_id);
        exit;
    }
    public function enable_2fa($member_id)
    {
        $this->comman_fun->delete('authentication', ['usercode' => $member_id]);
        $this->session->set_flashdata('show_msg', ['class' => 'true', 'msg' => 'Enable Two Factor Authentication Successfully']);
        header('Location: ' . file_path('admin') . '' . $this->uri->rsegment(1) . '/member/' . $member_id);
        exit;
    }
    public function unblock($member_id)
    {
        $this->comman_fun->delete('member_block', ['usercode' => $member_id]);
        $data             = [];
        $data['is_block'] = 0;
        $this->comman_fun->update($data, 'membermaster', ['usercode' => $member_id]);
        $data = false;
        $this->session->set_flashdata('show_msg', ['class' => 'true', 'msg' => 'Member Unblock Successfully']);
        header('Location: ' . file_path('admin') . '' . $this->uri->rsegment(1) . '/member/' . $member_id);
        exit;
    }
    //
    ...
    {
        $data = ['usercode' => $member_id, 'status' => 'Active', 'create_date' => date('Y-m-d h:i:s')];
        $this->comman_fun->addItem($data, 'member_block');
        $data                    = false;
        $data_update             = [];
        $data_update['is_block'] = 1;
        $this->comman_fun->update($data_update, 'membermaster', ['usercode' => $member_id]);
        $data_update = false;
        $this->session->set_flashdata('show_msg', ['class' => 'true', 'msg' => 'Member block Successfully']);
        header('Location: ' . file_path('admin') . '' . $this->uri->rsegment(1) . '/member/' . $member_id);
        exit;
    }
    public function verify_email_id($member_id)
    {
        ...
        $this->comman_fun->update($data, 'membermaster', ['usercode' => $member_id]);
        $data = false;
        $this->session->set_flashdata('show_msg', ['class' => 'true', 'msg' => 'Member Verify Email Successfully']);
        header('Location: ' . file_path('admin') . '' . $this->uri->rsegment(1) . '/member/' . $member_id);
        exit;
    }
}
