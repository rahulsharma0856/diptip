...
Class Comment_model extends CI_Model{
	...
	function edit_comment($comment_id, $result){
	    ...
	    $this->comman_fun->update($data, 'social_comments', array('id'=>$comment_id));
	...
	function getPostComments($post_id, $start_from){
	...
	function do_unlike_comment($comment_id){
			
			$this->comman_fun->delete('social_likes',array('comment_id'=>$comment_id,'usercode'=>user_session('usercode')));
			
			return true;
					
	}
		
	function do_like_comment($comment_id){
		...
		if(!$this->isMemberLikeComment($comment_id)){
			...
			//notification
			$cresult 		= $this->comman_fun->get_table_data('social_comments',array('id'=>$comment_id));
			
			if(!isset($cresult[0]['post_id']) or $cresult[0]['post_id']==0)
			{
				$cresult2 		= $this->comman_fun->get_table_data('social_comments',array('id'=>$cresult[0]['comment_id']));
	...
	function isMemberLikeComment($comment_id){
	...
	function countCommentTotalLikes($comment_id){
	...
	function countTotalReplyByID($comment_id)
	{
		$this -> db -> select('count(c2.id) as tot_reply'); //new
	...
	function getReplyById($comment_id){
	...
	function getAllReplyById($comment_id,$start_from){
	...
	function getPostCommentsPerticuler($post_id = NULL, $start_id = NULL){
	...
	function getPostPerticulerCommentById($post_id = NULL, $id = NULL){
	...
	function getPerticulerReplyById($comment_id = NULL, $start_id = NULL){
